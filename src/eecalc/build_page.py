...
<div class="subpanel" id="sub-cv">
  ...
  <div class="card">
    ...
    <div class="cardrow">
    <div class="fields">
      <div class="field"><label for="dbm-p">Power (dBm)</label><input id="dbm-p" inputmode="decimal" placeholder="e.g. 0"></div>
      <div class="field"><label for="dbm-w">Power (W)</label><input id="dbm-w" inputmode="decimal" placeholder="e.g. 1m"></div>
      <div class="field"><label for="dbm-z">Impedance (&Omega;)</label><input id="dbm-z" inputmode="decimal" placeholder="50"></div>
      <div class="field"><label for="dbm-vrms">V rms</label><input id="dbm-vrms" inputmode="decimal" placeholder="e.g. 223.6m"></div>
      <div class="field"><label for="dbm-vpk">V peak</label><input id="dbm-vpk" inputmode="decimal" placeholder="e.g. 316.2m"></div>
      <div class="field"><label for="dbm-vpp">V peak-to-peak</label><input id="dbm-vpp" inputmode="decimal" placeholder="e.g. 632.5m"></div>
    </div>
    <div class="diagram">
      <svg class="schem" width="250" height="150" viewBox="0 0 250 150" role="img" aria-label="A sine wave across a load, with the rms level, the peak and the peak-to-peak swing marked">
      <path class="wire" d="M18 74 H240"/>
      <path class="wire" d="M18 74 Q 40 18 62 74 T 106 74 T 150 74 T 194 74"/>
      <path class="wire opt" d="M18 36 H240"/>
      <text x="202" y="32">rms</text>
      <path class="wire" d="M40 20 V74 M34 20 H46 M34 74 H46"/>
      <text x="26" y="14">peak</text>
      <path class="wire" d="M84 20 V128 M78 20 H90 M78 128 H90"/>
      <text x="94" y="144">peak-to-peak</text>
    </svg>
      <p class="key">One cycle of a sine across the load. The rms level is the dashed line; peak and peak-to-peak are measured from it and from trough to crest. The sine is where the root-two factors come from &mdash; no other waveform has them.</p>
      <p class="plate">V<sub>rms</sub> = sqrt(P&middot;Z) &middot; V<sub>pk</sub> = sqrt(2)&middot;V<sub>rms</sub> &middot; V<sub>pp</sub> = 2&middot;V<sub>pk</sub></p>
    </div>
    </div>
    <dl class="results" id="dbm-out"></dl>
    <p class="note">Type into any one box and the rest follow. dBm is an absolute power, so it only becomes a voltage once the impedance is fixed &mdash; 50 &Omega; unless you say otherwise, 75 &Omega; for video and cable, 600 &Omega; for legacy audio lines. <b>The peak and peak-to-peak figures assume a sine wave.</b> For noise, a modulated carrier or any other waveform the crest factor differs and only the rms figure follows from the power; digital modulation routinely peaks 3 to 10 dB above its average, which is exactly what clips an amplifier that looked fine on paper.</p>
    <button class="reset" data-reset="dbm">Reset</button>
  </div>
<div class="card">
    <h3>Ratio units</h3>
    <div class="fields">
