...
/* A complex impedance written the way it is written on paper: one unit, and
   the j in front of the reactance rather than trailing the whole expression.
   Both parts share a prefix chosen from the larger, so 58.5 - j6.779 Ohm reads
   as one quantity instead of two unrelated ones. */
function fmtZ(re, im) {
  if (!isFinite(re) || !isFinite(im)) return "\u2014";
  const m = Math.max(Math.abs(re), Math.abs(im));
  const PRE = [[1e9, "G"], [1e6, "M"], [1e3, "k"], [1, ""], [1e-3, "m"], [1e-6, "\u00b5"]];
  let f = 1, pre = "";
  for (const [fac, pfx] of PRE) { if (m >= fac * 0.9999995) { f = fac; pre = pfx; break; } }
  const part = function (v) {
    let str = (v / f).toPrecision(4);
    if (str.indexOf("e") === -1 && str.indexOf(".") !== -1) str = str.replace(/\.?0+$/, "");
    return str;
  };
  return part(re) + (im < 0 ? " \u2212 j" : " + j") + part(Math.abs(im)) + " " + pre + "\u03a9";
}
...
/* ---------- dBm and volts ----------
   ...
   dBm is an absolute power referred to a milliwatt, so it is only a voltage
   once an impedance is named. Everything here goes through watts as the
   canonical quantity, which keeps one conversion path rather than five.

   The root-two factors are a sine-wave assumption and nothing more. The card
   says so, because the number that actually clips an amplifier is the peak of
   whatever waveform you really have, not of the sine it would be if life were
   simpler. */

function calcDbm() {
  const IDS = ["dbm-p", "dbm-w", "dbm-vrms", "dbm-vpk", "dbm-vpp"];
  /* Whatever the user typed wins; anything this card wrote is fair game to
     overwrite. Read that before clearing, or our own answers look like input. */
  const typed = IDS.filter(function (id) {
    const el = document.getElementById(id);
    return el && el.value.trim() !== "" && !el.classList.contains("computed");
  });
  clearComputed(IDS);
  const z = numOr("dbm-z", 50, 1e-12);
  if (!isFinite(z)) { render("dbm-out", []); return; }
  if (!typed.length) { render("dbm-out", []); return; }

  const src = typed[0];
  const v = val(src);
  if (!isFinite(v)) { render("dbm-out", []); return; }

  let watts;
  if (src === "dbm-p") watts = Math.pow(10, v / 10) / 1000;
  else if (src === "dbm-w") watts = v;
  else if (src === "dbm-vrms") watts = v * v / z;
  else if (src === "dbm-vpk") watts = (v / Math.SQRT2) * (v / Math.SQRT2) / z;
  else watts = (v / (2 * Math.SQRT2)) * (v / (2 * Math.SQRT2)) / z;

  if (!(watts > 0)) {
    render("dbm-out", [["", "Power must be above zero &mdash; dBm has no value at zero power.", "err"]]);
    return;
  }

  const vrms = Math.sqrt(watts * z);
  const out = { "dbm-p": 10 * Math.log10(watts * 1000), "dbm-w": watts,
                "dbm-vrms": vrms, "dbm-vpk": vrms * Math.SQRT2,
                "dbm-vpp": 2 * Math.SQRT2 * vrms };
  IDS.forEach(function (id) { if (id !== src) setComputed(id, out[id]); });

  const rows = [
    ["Into", fmt(z, "\u03a9")],
    ["Power", out["dbm-p"].toFixed(2) + " dBm (" + fmt(watts, "W") + ")"],
    ["Voltage", fmt(vrms, "V") + " rms \u00b7 " + fmt(out["dbm-vpk"], "V") + " peak \u00b7 " +
     fmt(out["dbm-vpp"], "V") + " peak-to-peak"],
    /* dBuV is the EMC unit for the same thing; the offset is 90 + 10log10(Z),
       which is the familiar +107 at 50 ohms */
    ["As dB&micro;V", (out["dbm-p"] + 90 + 10 * Math.log10(z)).toPrecision(5) + " dB&micro;V" +
     (Math.abs(z - 50) < 1e-9 ? " \u2014 dBm + 107 at 50 \u03a9" : "")],
    ["", "Peak and peak-to-peak assume a sine. For anything else only the rms figure follows from the " +
     "power, and the real peak depends on the waveform's crest factor.", ""]
  ];
  render("dbm-out", rows);
}
...
/* ---------- flex hatched plane ----------
   ...
   A folding flex cannot carry a solid return plane, so the plane is cross
   hatched. Everything below follows from the pattern itself; nothing here
   predicts impedance, which needs the stack-up and a field solver.

   Model: two families of parallel tracks of width w, each family spaced p
   apart measured perpendicular to the tracks, set at +/-phi to the signal
   direction. That covers the ordinary 45 degree diamond and the elongated
   diamonds the IPC APEX 2018 work recommends, and it is the parameterisation
   the fill-factor algebra is actually written in.

   Fill factor. The opening is bounded by the inner edges of four tracks, so
   the open fraction is the product of the two independent gap ratios:

       open = (1 - w/p1)(1 - w/p2),   fill = 1 - open

   For a symmetric hatch that is 1 - (1 - w/p)^2. Note what is absent: the
   angle. Skewing the lattice shears the unit cell and the opening by the same
   factor, so the ratio is untouched. Fill is a function of w/p alone, which is
   why w/p = 0.293 gives 50 % copper at any hatch angle.

   Sheet resistance. Each family conducts only along its own axis, so in the
   homogenised limit the sheet conductance is the sum of two rank-one tensors,
   sigma*t*(w/p) along each track direction. Along the signal axis that gives

       Rs_eff / Rs_solid = (p/w) / (2 cos^2 phi)

   which is p/w at 45 degrees. Worth stating plainly because it is widely got
   wrong: a 50 % fill does NOT double the return resistance. At w/p = 0.293 the
   factor is 3.4, because the transverse family carries no net current along
   the signal and so contributes none of its copper to the return path. */

const HATCH_RULES = {
  /* IPC APEX 2018, Gisin/Moghe, "Minimizing Signal Degradation in Flexible PCB
     Microstrip and Stripline Transmission Lines That Use Cross-Hatched Return
     Planes", design guidelines B, C and the >50 % fill finding. */
  longest_mm: 1.27,
  aspect: 0.5,
  minFill: 0.5
};
...
function calcHatch() {
  clearComputed(["hx-w", "hx-p"]);
  const phiDeg = numOr("hx-ang", 45, 1, 89);
  const mode = document.getElementById("hx-pmode").value;
  const cu = val("hx-cu");
  let w = valDim("hx-w"), p = valDim("hx-p");
  const fPct = val("hx-f");

  if (!isFinite(phiDeg)) { render("hx-out", []); return; }
  const phi = phiDeg * Math.PI / 180, sp = Math.sin(phi), cp = Math.cos(phi);

  /* The CAD grid is the lattice period along the board axis, not the spacing
     between tracks, and it is the larger of the two: p = grid * sin(phi), so
     root two smaller at 45 degrees. Mixing them up is quiet rather than loud -
     a 0.15 mm track on a 0.5 mm grid reads as 51 % fill when it is really 67 %
     - so the card asks which one was typed instead of guessing. */
  const perpToAxis = function (v) { return v / sp; };
  if (isFinite(p) && mode === "axis") p = p * sp;

  const rows = [];
  const F = isFinite(fPct) ? fPct / 100 : NaN;
  if (isFinite(F) && !(F > 0 && F < 1)) {
    render("hx-out", [["", "Fill has to sit between 0 and 100 % &mdash; at 100 % the plane is solid and needs no hatch.", "err"]]);
    return;
  }

  /* Solve for whichever of the three was left out. */
  let solved = null;
  if (isFinite(w) && isFinite(p)) {
    /* both given; any fill figure is an output, not a constraint */
  } else if (isFinite(F) && isFinite(p)) {
    w = p * (1 - Math.sqrt(1 - F)); solved = "hx-w";
  } else if (isFinite(F) && isFinite(w)) {
    p = w / (1 - Math.sqrt(1 - F)); solved = "hx-p";
  } else {
    render("hx-out", [["", "Give a track width and a pitch, or a target fill and one of them.", ""]]);
    return;
  }
  if (!(w > 0) || !(p > 0)) { render("hx-out", []); return; }
  if (w >= p) {
    render("hx-out", [["", "The track is at least as wide as the pitch, so the tracks merge and the plane is solid. Reduce the width or open the pitch.", "err"]]);
    return;
  }
  if (solved === "hx-w") setComputed("hx-w", +w.toPrecision(4));
  if (solved === "hx-p") setComputed("hx-p", +(mode === "axis" ? perpToAxis(p) : p).toPrecision(4));

  const r = w / p, g = p - w;
  const fill = 1 - (1 - r) * (1 - r);

  /* Extents of one repeat and of one opening, resolved along and across the
     signal. For phi below 45 the cell is longer along the signal, which is the
     orientation the paper asks for. */
  const cellAlong = p / sp, cellAcross = p / cp;
  const openAlong = g / sp, openAcross = g / cp;
  const longest = Math.max(cellAlong, cellAcross);
  const shortest = Math.min(cellAlong, cellAcross);

  rows.push(["Copper fill", (fill * 100).toFixed(1) + " % &mdash; " + (100 - fill * 100).toFixed(1) + " % of the plane etched away"]);
  rows.push(["Width / pitch", r.toFixed(3) + (Math.abs(r - 0.29289) < 0.002 ? " &mdash; the 50 % landmark" : "")]);
  rows.push(["Track and gap", w.toPrecision(3) + " mm track, " + g.toPrecision(3) + " mm gap, " + p.toPrecision(3) + " mm pitch"]);
  rows.push(["As a CAD grid", perpToAxis(p).toPrecision(3) + " mm along the signal axis, " + (p / cp).toPrecision(3) + " mm across"]);
  rows.push(["Opening", openAlong.toPrecision(3) + " mm along the signal &times; " + openAcross.toPrecision(3) + " mm across"]);
  rows.push(["Repeat", cellAlong.toPrecision(3) + " mm along the signal &times; " + cellAcross.toPrecision(3) + " mm across"]);

  /* Return resistance. The transverse family does not help, which is the
     whole reason the penalty is so much worse than 1/fill suggests. */
  const rsRatio = (p / w) / (2 * cp * cp);
  const rsAcross = (p / w) / (2 * sp * sp);
  let rsRow = rsRatio.toPrecision(3) + "&times; solid along the signal, " + rsAcross.toPrecision(3) + "&times; across";
  if (isFinite(cu) && cu > 0) {
    /* 1 oz is 34.79 um of copper; 1.724e-8 ohm m at 20 C */
    const t = cu * 34.79e-6;
    const rsSolid = 1.724e-8 / t;
    rsRow += " — " + fmt(rsSolid * rsRatio, "\u03a9/sq") + " on " + cu + " oz copper";
  }
  rows.push(["Sheet resistance", rsRow]);
  rows.push(["", "A hatch costs more return resistance than its fill suggests: only the tracks that run along the current carry it, so the penalty is p/w scaled by the angle, not 1/fill.", ""]);

  /* The published geometric guidelines, checked rather than recited. */
  const okFill = fill >= HATCH_RULES.minFill;
  rows.push(["Fill guideline", okFill
    ? "at or above the 50 % that the RF work recommends"
    : "below 50 % &mdash; usable for low speed, but the RF studies stop recommending it here",
    okFill ? "good" : "warn"]);

  const okLong = longest <= HATCH_RULES.longest_mm;
  rows.push(["Repeat size guideline", "longest repeat " + longest.toPrecision(3) + " mm, "
    + (okLong ? "within" : "above") + " the 1.27 mm (50 mil) limit",
    okLong ? "good" : "warn"]);

  const aspect = shortest / longest;
  const okAsp = aspect <= HATCH_RULES.aspect;
  rows.push(["Elongation guideline", "short/long = " + aspect.toFixed(2) + " at " + phiDeg.toFixed(0) + "&deg;, "
    + (okAsp ? "meeting" : "missing") + " the &le; 0.5 the paper asks for"
    + (okAsp ? "" : " &mdash; needs " + (Math.atan(0.5) * 180 / Math.PI).toFixed(1) + "&deg; or shallower"),
    okAsp ? "good" : "warn"]);
  if (Math.abs(phiDeg - 45) < 0.5) {
    rows.push(["", "At 45&deg; the pattern is a square rotated, so it is equally long in both directions. The paper prefers a diamond stretched along the signal, which means a shallower angle.", ""]);
  }
  rows.push(["Model", "Pattern geometry only. Fill, openings and sheet resistance are exact for this lattice; the impedance of a trace over it is not a closed form and needs a solver."]);
  render("hx-out", rows);
}

/* ---------- S11 to impedance ----------

   A one-port reflection measurement gives the complex ratio of the returning
   wave to the outgoing one at some reference plane. The bilinear map back to
   impedance is exact:

       Z = Z0 (1 + G) / (1 - G)

   The thing worth being careful about is what happens when only |S11| is
   known. Magnitude alone does NOT determine Z. Every load on the circle of
   constant |G| produces the same return loss, the same VSWR and the same
   mismatch loss, so the honest answer is that circle. Mapped into the Z plane
   it stays a circle, centred on the real axis at

       centre = Z0 (1 + p^2) / (1 - p^2),   radius = Z0 * 2p / (1 - p^2)

   which is where the R span and the greatest possible |X| in the output come
   from. A calculator that returns one resistance from |S11| is silently
   choosing the point where that circle crosses the real axis on the high side;
   the low-side crossing fits the same measurement exactly as well. */

function calcS11() {
  const IDS = ["s11-db", "s11-lin"];
  /* Whichever of the pair the user typed is the source; the other is ours to
     overwrite. Read that before clearing or our own answer looks like input. */
  const typed = IDS.filter(function (id) {
    const el = document.getElementById(id);
    return el && el.value.trim() !== "" && !el.classList.contains("computed");
  });
  ...
  const z0 = numOr("s11-z0", 50, 1e-12);
  if (!isFinite(z0)) { render("s11-out", []); return; }
  if (!typed.length) { render("s11-out", []); return; }

  const src = typed[0];
  const v = val(src);
  if (!isFinite(v)) { render("s11-out", []); return; }

  let rho;
  if (src === "s11-db") rho = Math.pow(10, v / 20);
  else rho = Math.abs(v);
  if (!(rho >= 0)) { render("s11-out", []); return; }

  const rows = [];
  if (rho === 0) {
    setComputed(src === "s11-db" ? "s11-lin" : "s11-db", src === "s11-db" ? 0 : -Infinity);
    rows.push(["Match", "Perfect &mdash; nothing reflected, so the load is exactly Z<sub>0</sub> = " + fmt(z0, "\u03a9")]);
    rows.push(["VSWR", "1.000 : 1"]);
    render("s11-out", rows);
    return;
  }
  if (src === "s11-db") setComputed("s11-lin", +rho.toPrecision(5));
  else setComputed("s11-db", +(20 * Math.log10(rho)).toPrecision(5));

  const dB = 20 * Math.log10(rho);
  rows.push(["Reflection coefficient", "|\u0393| = " + rho.toPrecision(4) + " = " + dB.toFixed(2) + " dB"]);

  if (rho >= 1) {
    /* More came back than went out. Passive loads cannot do this, so it is a
       calibration fault, an oscillating active device, or a genuine negative
       resistance. Say which rather than returning a negative resistance with
       no comment. */
    rows.push(["Not passive", rho > 1
      ? "More power returns than was sent. A passive load cannot do that &mdash; suspect the calibration, or an active device with negative resistance."
      : "All of the power returns: an ideal open, short or pure reactance. The impedance is on the edge of the Smith chart.",
      "warn"]);
    if (rho > 1) { render("s11-out", rows); return; }
  } else {
    rows.push(["Return loss", (-dB).toFixed(2) + " dB"]);
    rows.push(["VSWR", ((1 + rho) / (1 - rho)).toFixed(4) + " : 1"]);
    rows.push(["Power reflected", (rho * rho * 100).toPrecision(3) + " % &mdash; mismatch loss "
      + (-10 * Math.log10(1 - rho * rho)).toFixed(3) + " dB"]);
  }

  const phDeg = val("s11-ph");
  if (isFinite(phDeg)) {
    /* Phase known: the impedance is determined. */
    const ph = phDeg * Math.PI / 180;
    const gr = rho * Math.cos(ph), gi = rho * Math.sin(ph);
    const dr = 1 - gr, di = -gi;
    const den = dr * dr + di * di;
    if (den < 1e-18) {
      rows.push(["Impedance", "\u0393 = 1 exactly, which is an ideal open circuit &mdash; the impedance is unbounded.", "warn"]);
      render("s11-out", rows);
      return;
    }
    const nr = 1 + gr, ni = gi;
    const R = z0 * (nr * dr + ni * di) / den;
    const X = z0 * (ni * dr - nr * di) / den;
    rows.push(["\u0393 rectangular", gr.toPrecision(4) + (gi < 0 ? " - " : " + ") + Math.abs(gi).toPrecision(4) + "j"]);
    rows.push(["Impedance", fmtZ(R, X), "good"]);
    rows.push(["In polar form", fmt(Math.sqrt(R * R + X * X), "\u03a9") + " \u2220 "
      + (Math.atan2(X, R) * 180 / Math.PI).toFixed(2) + "&deg;"]);
    if (R < 0) {
      rows.push(["Negative resistance", "The real part is below zero, which no passive load has. Check the calibration and the reference plane.", "warn"]);
    }
    /* The reactance as a part you could actually fit. */
    const f = val("s11-f");
    if (isFinite(f) && f > 0 && Math.abs(X) > 1e-12) {
      rows.push(["Series equivalent at " + fmt(f, "Hz"), X > 0
        ? fmt(X / (2 * Math.PI * f), "H") + " in series with " + fmt(R, "\u03a9")
        : fmt(1 / (2 * Math.PI * f * Math.abs(X)), "F") + " in series with " + fmt(R, "\u03a9")]);
    } else if (Math.abs(X) > 1e-12) {
      rows.push(["", "Give a frequency and the reactance is reported as the inductance or capacitance it corresponds to.", ""]);
    }
  } else if (rho < 1) {
    /* Phase unknown: report the circle, not a point. */
    const c = z0 * (1 + rho * rho) / (1 - rho * rho);
    const r = z0 * 2 * rho / (1 - rho * rho);
    rows.push(["Impedance", "Not determined by the magnitude alone &mdash; add the phase.", "warn"]);
    rows.push(["The load lies on", "a circle centred at " + fmt(c, "\u03a9") + " of radius " + fmt(r, "\u03a9")]);
    rows.push(["Purely resistive cases", fmt(z0 * (1 - rho) / (1 + rho), "\u03a9") + " or " + fmt(z0 * (1 + rho) / (1 - rho), "\u03a9")
      + " &mdash; the two points where that circle meets the real axis"]);
    rows.push(["Reactance could be", "anything up to \u00b1" + fmt(r, "\u03a9") + " at the extremes of the circle"]);
    rows.push(["", "Return loss, VSWR and mismatch loss are all fixed by the magnitude, so those figures above are exact. The impedance is not.", ""]);
  }
  rows.push(["Model", "Exact for a linear one-port at the reference plane the measurement was calibrated to. "
    + "Moving the plane rotates the phase without changing the magnitude, so de-embed first."]);
  render("s11-out", rows);
}
...
const HELP = {
  "s11": {
    "title": "S11 to impedance",
    "body": "<p>A one-port measurement sends a wave at the device and reports what comes back, as a complex ratio at the reference plane the calibration established:</p><span class=\"eq\">S11 = &Gamma; = b / a</span><p>The map from that ratio to impedance is a bilinear transform, and it is exact rather than approximate:</p><span class=\"eq\">Z = Z<sub>0</sub> (1 + &Gamma;) / (1 &minus; &Gamma;) &nbsp;&nbsp; &Gamma; = (Z &minus; Z<sub>0</sub>) / (Z + Z<sub>0</sub>)</span><p>&Gamma; = 0 is a perfect match, &Gamma; = +1 an open, &Gamma; = &minus;1 a short, and |&Gamma;| = 1 at any phase is a pure reactance. The whole right half of the impedance plane maps inside the unit circle, which is what a Smith chart is.</p><p><b>Why the phase is not optional, whatever a magnitude-only calculator implies.</b> |S11| fixes how much came back but says nothing about where the load sits around the chart. Every impedance on the circle of constant |&Gamma;| gives an identical return loss, VSWR and mismatch loss. Mapped into the Z plane that circle is centred on the real axis at</p><span class=\"eq\">centre = Z<sub>0</sub>(1 + &rho;&sup2;)/(1 &minus; &rho;&sup2;) &nbsp;&nbsp; radius = 2 Z<sub>0</sub> &rho; / (1 &minus; &rho;&sup2;)</span><p>At &minus;20 dB into 50 &Omega; that is a circle centred on 51.0 &Omega; of radius 10.1 &Omega;. So the load could be 61.1 &Omega;, or 40.9 &Omega;, or 49.0 + 9.9j &Omega;, or anything else on that circle. A tool that hands back &ldquo;61 &Omega;&rdquo; has chosen the high-side real-axis crossing and not told you. This card gives you the circle until you supply a phase.</p><p><b>The scalar figures, which the magnitude does fix.</b></p><span class=\"eq\">VSWR = (1 + &rho;)/(1 &minus; &rho;) &nbsp;&nbsp; return loss = &minus;20&middot;log<sub>10</sub>&rho; &nbsp;&nbsp; mismatch loss = &minus;10&middot;log<sub>10</sub>(1 &minus; &rho;&sup2;)</span><p>Return loss is quoted as a positive number by convention while S11 in dB is negative; they are the same measurement with opposite signs, and mixing them up is the most common slip in a link budget. Mismatch loss is the part that matters for power transfer: at &minus;10 dB return loss you are still delivering 90 % of the power, which is why chasing &minus;20 dB on an antenna rarely repays the effort the last few dB cost.</p><p><b>The reference plane is part of the measurement.</b> Moving it along a line rotates &Gamma; around the chart at two degrees per degree of electrical length, leaving |&Gamma;| untouched. So the magnitude, and therefore VSWR and return loss, are the same anywhere along a lossless line, while the impedance is completely different. If the calibration plane is at the connector and the part is 10 mm further on, the impedance this card reports is the impedance at the connector, not at the part. De-embed first, or calibrate to the plane you care about.</p><p><b>What a negative resistance means.</b> If the computed R comes out below zero, or |S11| exceeds 1, more power returned than was sent. No passive load does that. In practice it is a stale calibration, a cable moved since calibration, or an amplifier on the edge of oscillating &mdash; in which case the negative resistance is real and is telling you something important.</p>"
  },
  "hx": {
    "title": "Flex hatched plane",
    "body": "<p>A flex circuit that folds cannot carry a solid return plane. Copper work-hardens where it bends, and a continuous sheet on the outside of a bend is the first thing to crack. Removing most of the copper in a regular lattice leaves the plane flexible, and leaves you with a return path that is worse than solid in every respect except the one that matters.</p><p><b>The pattern.</b> Two families of parallel tracks, width w, spaced p apart measured <i>perpendicular to the tracks</i>, crossing at some angle. The card takes that angle as φ, the tilt of each family away from the signal direction, so φ = 45° is the ordinary diamond and a smaller φ is a diamond stretched along the signal.</p><p><b>Fill factor.</b> Each opening is bounded by the inner edges of four tracks, so the open fraction is the product of the two gap ratios:</p><span class=\"eq\">open = (1 − w/p)² &nbsp;&nbsp; fill = 1 − (1 − w/p)²</span><p>Notice what is <i>not</i> in that expression: the angle. Skewing the lattice shears the unit cell and the opening inside it by the same factor, so the ratio survives untouched. Fill depends on w/p alone. Setting it to ½ gives w/p = 1 − 1/sqrt(2) = 0.293, which is where the 29.3 % figure quoted in fabricator notes comes from.</p><p><b>The pitch trap.</b> CAD tools specify a hatch by its <i>grid</i>, which is the lattice period measured along a board axis, not the spacing between tracks. For a 45° hatch the two differ by sqrt(2), and the grid is the larger of the pair. Feed a 0.5 mm grid into the formula as if it were the perpendicular pitch, with a 0.15 mm track, and you compute 51 % fill for a pattern that actually has 67 % — the error understates the copper, and it understates the bend relief with it. The card has a selector for which one you typed; set it honestly.</p><p><b>Return resistance, and why it is worse than it looks.</b> Only the tracks running along the current carry it — the transverse family sits at right angles to the flow and contributes nothing. Summing the two families as rank-one conductance tensors and inverting along the signal axis gives</p><span class=\"eq\">R<sub>s,eff</sub> / R<sub>s,solid</sub> = (p/w) / (2·cos²φ)</span><p>which is simply p/w at 45°. So a plane at 50 % fill does not have twice the sheet resistance — it has 3.4 times, because half the remaining copper is pointing the wrong way. That matters for a return path shared with power, and it is the usual reason a hatched flex measures a worse drop than the designer expected.</p><p><b>What the geometry cannot tell you.</b> The impedance of a trace over a hatched plane. It is always higher than over solid copper, because the return current detours around the openings, which lengthens the loop and lowers the coupling. There is no closed form for it: the structure is periodic rather than uniform, so strictly it is not a transmission line at all, and a field solver with the real stack-up is the only honest route. Treat any calculator that hands you a hatched-plane impedance from a formula with suspicion.</p><p><b>The published guidelines</b> the card checks come from Gisin and Moghe's IPC APEX 2018 study of cross-hatched return planes. Keep the longest repeat at or under 1.27 mm (50 mil), because the periodic structure resonates at a frequency inversely proportional to the hole size. Keep the short axis at or under half the long one, and put the long axis parallel to the signal, so the return current runs more nearly alongside the trace instead of zig-zagging. Stay at or above 50 % fill for RF. And keep hatched runs short: the paper found lengths under about 2.5 cm behave far better than those over 8 to 12 cm.</p><p><b>Two things the card does not model but you still have to do.</b> Register single-ended traces between hatch intersections and differential pairs straddling them — misregistration between the trace and the lattice makes the two halves of a pair see different amounts of copper, which shows up as skew. And never hatch a plane that is doing duty as part of the power distribution network as well as a signal return.</p>"
  },
  "ohm": {
  ...
  "dbm": {"title": "dBm and volts", "body": "<p>dBm is an absolute power referred to one milliwatt, not a ratio:</p><span class=\"eq\">P = 10<sup>(dBm/10)</sup> / 1000 watts</span><p>It becomes a voltage only once you name an impedance, because power and voltage are related through it:</p><span class=\"eq\">V<sub>rms</sub> = sqrt(P &middot; Z) &nbsp;&nbsp; P = V<sub>rms</sub>&sup2; / Z</span><p>50 &Omega; is the default because almost all RF test gear is built to it. 75 &Omega; is video and cable television, 600 &Omega; is legacy audio lines, and getting this wrong shifts every voltage by half the impedance ratio in dB &mdash; 50 to 75 &Omega; is 1.76 dB.</p><p><b>Anchors worth memorising.</b> 0 dBm into 50 &Omega; is 224 mV rms, and +13 dBm is almost exactly 1 V rms into the same. Every 3 dB doubles the power and so multiplies voltage by 1.41; every 6 dB doubles the voltage. A 20 dB step is ten times the voltage and a hundred times the power.</p><p><b>The three voltages.</b> RMS is the one that carries the power and the only one that follows from it directly. The others come from the waveform:</p><span class=\"eq\">V<sub>pk</sub> = sqrt(2) &middot; V<sub>rms</sub> &nbsp;&nbsp; V<sub>pp</sub> = 2 &middot; V<sub>pk</sub> = 2&middot;sqrt(2) &middot; V<sub>rms</sub></span><p><b>Those root-two factors are a sine-wave assumption</b>, and it is the assumption that catches people out. The ratio of peak to rms is the crest factor: 1.41 for a sine, 1.0 for a square wave, about 3 to 4 for Gaussian noise, and anywhere from 3 to 10 dB for digitally modulated carriers. An OFDM signal at +20 dBm average peaks near +30 dBm, so an amplifier sized from the average clips on the peaks and spatters the adjacent channel. When the waveform is not a sine, trust the rms figure and get the peak from the modulation's own crest factor.</p><p><b>dB&micro;V</b> is the same quantity in the units EMC work uses &mdash; 20&middot;log<sub>10</sub> of the rms voltage referred to a microvolt. The offset from dBm is 90 + 10&middot;log<sub>10</sub>(Z), which is the familiar +107 at 50 &Omega;. Emissions limits are quoted in dB&micro;V or dB&micro;V/m, so this is the bridge between a receiver's noise floor in dBm and a limit line on a test report.</p><p><b>What this does not cover.</b> dBm always needs a stated impedance, so a voltage measured across a high-impedance probe is not a dBm figure without knowing what the source actually drives. And a reading in dBm from a spectrum analyser is power in its resolution bandwidth, not total power &mdash; for noise you have to account for the bandwidth before comparing.</p>"},
  "pdn": {
...
const CALCS = {
  ...
  hx:  { calc: calcHatch, inputs: ["hx-w","hx-p","hx-pmode","hx-f","hx-ang","hx-cu"] },
  s11: { calc: calcS11, inputs: ["s11-z0","s11-db","s11-lin","s11-ph","s11-f"] },
  ...
  dbm: { calc: calcDbm, inputs: ["dbm-p","dbm-w","dbm-z","dbm-vrms","dbm-vpk","dbm-vpp"] }
};
