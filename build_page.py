...
<section class="panel" id="panel-div">
  ...
  <div class="card">
    <h3>Design one leg</h3>
    <div class="fields">
      <div class="field"><label for="dsn-vin">V<sub>in</sub> (V)</label><input id="dsn-vin" inputmode="decimal" placeholder="e.g. 12"></div>
      <div class="field"><label for="dsn-vout">Target V<sub>out</sub> (V)</label><input id="dsn-vout" inputmode="decimal" placeholder="e.g. 3.3"></div>
      <div class="field"><label for="dsn-known">Known leg</label>
        <select id="dsn-known">
          <option value="R1" selected>R1 (top)</option>
          <option value="R2">R2 (bottom)</option>
        </select>
      </div>
      <div class="field"><label for="dsn-rknown">Known value (&Omega;)</label><input id="dsn-rknown" inputmode="decimal" placeholder="e.g. 10k"></div>
      <div class="field"><label for="dsn-iload">Load current at midpoint (A, optional)</label><input id="dsn-iload" inputmode="decimal" placeholder="e.g. 50u"></div>
      <div class="field"><label for="dsn-series">Suggest from</label>
        <select id="dsn-series">
    ...
    <dl class="results" id="dsn-out"></dl>
    <p class="note">The load is modelled as a constant current drawn out of the midpoint, so the top leg carries the R2 current plus the load. Leave it empty for an unloaded divider.</p>
    <button class="reset" data-reset="dsn">Reset</button>
  </div>
  <div class="card">
...
/* Design one leg: Vin, target Vout, one known resistor, optional load current
   drawn from the midpoint. KCL at the midpoint: I(R1) = I(R2) + Iload, so
   R1 = (Vin-Vout)/(Vout/R2 + IL) and R2 = Vout/((Vin-Vout)/R1 - IL).
   With both legs fixed the loaded output is Vout = (Vin - R1*IL)/(1 + R1/R2). */
function calcDesign() {
  const vin = val("dsn-vin"), vout = val("dsn-vout"), rk = val("dsn-rknown");
  const ilRaw = val("dsn-iload");
  const il = isFinite(ilRaw) ? ilRaw : 0;
  if (![vin, vout, rk].every(isFinite)) { render("dsn-out", []); return; }
  if (!(vin > 0) || !(vout > 0) || vout >= vin) { render("dsn-out", [["", "Needs 0 < Vout < Vin.", "err"]]); return; }
  if (!(rk > 0)) { render("dsn-out", [["", "The known resistor must be positive.", "err"]]); return; }
  if (il < 0) { render("dsn-out", [["", "Load current must be zero or positive.", "err"]]); return; }
  const topKnown = document.getElementById("dsn-known").value === "R1";
  let r1, r2;
  if (topKnown) {
    r1 = rk;
    const i2 = (vin - vout) / r1 - il;      // current left for the bottom leg
    if (i2 <= 0) {
      render("dsn-out", [["", "The load alone drops " + fmt(il * r1, "V") + " across R1 — Vout is unreachable. Use a smaller R1 or less load.", "err"]]);
      return;
    }
    r2 = vout / i2;
  } else {
    r2 = rk;
    r1 = (vin - vout) / (vout / r2 + il);
  }
  const series = document.getElementById("dsn-series").value;
  const values = seriesValues(series, -1, 7);          // 100 mohm .. 97.6 Mohm
  const rCalc = topKnown ? r2 : r1;                    // the computed leg
  const rStd = snap(values, rCalc);
  const sr1 = topKnown ? r1 : rStd, sr2 = topKnown ? rStd : r2;
  const voStd = (vin - sr1 * il) / (1 + sr1 / sr2);
  const i1 = (vin - voStd) / sr1, i2s = voStd / sr2;
  const rows = [
    ["Exact " + (topKnown ? "R2" : "R1"), fmt(rCalc, "Ω")],
    ["Nearest " + series, fmt(rStd, "Ω")],
    ["V<sub>out</sub> with " + series + " value", fmt(voStd, "V") + " (" + ((voStd - vout) / vout * 100).toFixed(3) + " %)"],
    ["Current in R1", fmt(i1, "A")],
    ["Current in R2", fmt(i2s, "A")],
    ["P in R1 / R2", fmt(i1 * i1 * sr1, "W") + " / " + fmt(i2s * i2s * sr2, "W")]
  ];
  if (il > 0) {
    const ratio = i2s / il;
    rows.push(["Bleed / load ratio", ratio.toPrecision(3) + (ratio < 10 ? " — a soft divider; Vout moves if the load varies" : "")]);
  }
  render("dsn-out", rows);
}
...
const CALCS = {
  ...
  dsn: { calc: calcDesign, inputs: ["dsn-vin","dsn-vout","dsn-known","dsn-rknown","dsn-iload","dsn-series"] },
