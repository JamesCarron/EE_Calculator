...
<section class="panel" id="panel-div">
  ...
  <p class="hint">R1 on top, R2 to ground; unloaded, V<sub>out</sub> = V<sub>in</sub> &middot; R2 / (R1 + R2). The solver fills in whichever of the four values is missing; the finder searches standard E-series values for the pair closest to a target output.</p>
  <div class="card">
...
function calcDivider() {
  const vin = val("div-vin"), vout = val("div-vout"), r1 = val("div-r1"), r2 = val("div-r2");
  const ilRaw = val("div-iload");
  ...
  if (count < 3) {
    render("div-out", count ? [["", "Enter " + (3 - count) + " more value" + (count === 2 ? "" : "s") + " — any three of V<sub>in</sub>, V<sub>out</sub>, R1, R2.", ""]] : []);
    return;
  }
  if (il < 0) { render("div-out", [["", "Load current must be zero or positive.", "err"]]); return; }
  if ((have.R1 && !(r1 > 0)) || (have.R2 && !(r2 > 0))) { render("div-out", [["", "Resistor values must be positive.", "err"]]); return; }
...
const CALCS = {
  ...
  div: { calc: calcDivider, inputs: ["div-vin","div-vout","div-r1","div-r2","div-iload"] },
  fnd: { calc: calcFinder, inputs: ["fnd-vin","fnd-vout","fnd-rtot"] },
...
document.getElementById("g-series").addEventListener("change", function () {
  calcDivider(); calcFinder(); calcLED();
});
