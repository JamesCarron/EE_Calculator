...
function calcBattery() {
  ...
  if (!isFinite(mah) || !(mah > 0) || !isFinite(vc) || !isFinite(usable) ||
      !isFinite(vfull) || !isFinite(vmin)) { render("bat-out", []); return; }

  const packV = vc * S;
  const packAh = mah / 1000 * P;
  const wh = packV * packAh;
  const rows = [
    ["Pack", S + "S" + P + "P at " + fmt(packV, "V") + " nominal" + (chem ? ", " + chem.name : "")],
  ...
  render("bat-out", rows);
  drawBatteryCurve(vfull, vc, vmin, S, chem ? chem.shape : "slope", usable);
}
