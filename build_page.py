...
function drawPad(hasZ2) {
  ...
  document.getElementById("pad-diagram").innerHTML = g.join("") +
    ...
    (hasZ2 ? ", which is the case you are solving." : "; give a second impedance to size one.") + "</p>";
}
...
/* logarithmic decade ticks between two frequencies */
function decadeTicks(lo, hi, unit) {
  const ticks = [];
  for (let d = Math.ceil(Math.log10(lo)); d <= Math.floor(Math.log10(hi)); d++) {
    ticks.push({ at: d, label: fmt(Math.pow(10, d), unit) });
  }
  return ticks;
}
...
function drawReactPlot(cap, ind) {
  ...
  const lo = Math.log10(centre) - 2.5, hi = Math.log10(centre) + 2.5;
  const N = 121;
  const series = [];
  const zc = [], zl = [];
  for (let i = 0; i < N; i++) {
  ...
  miniPlot("re-graph", {
    series: series,
    yMin: yLo, yMax: yHi,
    xTicks: decadeTicks(Math.pow(10, lo), Math.pow(10, hi), "Hz"),
    yTicks: yTicks,
    yUnit: "|Z|",
...
function drawFusePlot(aCmil, ta, kOn, tNow) {
  const N = 61, lo = -2, hi = 1.5;                 // 10 ms to about 30 s
