...
`src/eecalc/build_page.py` generates one self-contained `EE_Calculator.html` at the repo root. Since 2026-09-17 the page is three real files — `src/eecalc/templates/page.html`, `src/eecalc/static/page.css` and `src/eecalc/static/page.js` — which `load_page()` substitutes into one string at `{{page.css}}` and `{{page.js}}`. **Every change is an edit to one of those three followed by `pixi run build`** — never edit the generated HTML except to test something throwaway. Logic lives in JS because the page is interactive at runtime; Python only assembles it, so there is no second implementation and no parity test owed.
...
`src/eecalc/` is the package that builds the page — `templates/page.html`, `static/page.css` and `static/page.js` are the page itself and `build_page.py` assembles them. `tests/` checks it — `tests/js/` for the node suites, `tests/checks/` for the Python structural checkers, `tests/run_all.py` as the one runner — `scripts/` has the setup scripts and `where.py`, and `examples/` the worked examples. The generated `EE_Calculator.html` and its launcher stay at the repo root because they are the product; everything else is how it is made. Nothing the user owns lives in the checkout: `src/eecalc/paths.py` is the only module that knows where the openEMS cache and the generated models are, and nothing else may hard-code a path under `%LOCALAPPDATA%` or `Documents`.

Plans, specs, research and task summaries — including the Saturn/KiCad/3roam feature-comparison research and `Refactor_Plan.md` — live in `docs/private/`, a gitignored directory junction to `C:\Auterion\Tools\project-context\EE_Calculator\` rather than in the repo. See `C:\Auterion\Tools\Private_Context_Junctions.md` for why. The junction only works from inside this checkout; the real files are at that project-context path, editable directly if `docs/private/` is ever unavailable.
