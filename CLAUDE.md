...
`src/eecalc/` is the package that builds the page — `templates/page.html`, `static/page.css` and `static/page.js` are the page itself and `build_page.py` assembles them —, `tests/` checks it — `tests/js/` for the node suites, `tests/checks/` for the Python structural checkers, `tests/run_all.py` as the one runner — `scripts/` has the setup scripts and `where.py`, `docs/` the design documents and the refactor plan, and `examples/` the worked examples. The generated `EE_Calculator.html` and its launcher stay at the repo root because they are the product; everything else is how it is made. Nothing the user owns lives in the checkout: `src/eecalc/paths.py` is the only module that knows where the openEMS cache and the generated models are, and nothing else may hard-code a path under `%LOCALAPPDATA%` or `Documents`.
