...
Plans, specs, research and task summaries — including the Saturn/KiCad/3roam feature-comparison research and `Refactor_Plan.md` — live in `docs/private/` rather than in the repo. It is a gitignored directory junction to a private context folder held outside the checkout, so it is never part of a clone; the target path and the reasoning are in the maintainer's own notes, deliberately not recorded here.
